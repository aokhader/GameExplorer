...
/**
 * A move as the engine layer passes it around. Reversi collapses `from === to`.
 * Mobile's `LocalMove` is an alias of this, so the play loop and the analysis
 * layer describe a move exactly once.
 */
...
/**
 * What review needs from a game, on top of what the play adapter already
...
export interface AnalysisAdapter<S> {
   ...
   */
  lastMove(state: S): EngineMove | null;
  /** Side to move — the mover whose move gets graded. */

 ...
 * and web's fine-grained `eloDescription` blurb for the custom picker. Those
 * are presentation — a phone tile and a desktop row want different lengths —
 * and unifying them would be a visual change wearing a refactor's clothes.
 * Only the numbers and the canonical label live here, because only those two
 * can be *wrong* rather than merely different.
 *
 * Data only, per this directory's contract — so the naming *built* on these
 * tiers is not here either: `bots/strengthLabel.ts` turns a rating into the
 * word for it, and is the only thing that should.
 */

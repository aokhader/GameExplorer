...
export default function CheckersPlayPage() {
  ...
  return (
    <div className={`min-h-screen bg-slate-900 text-white pt-16 flex flex-col items-center px-4 py-6 ${inGame ? 'justify-start' : 'justify-center'}`}>
            ...
            <>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {TIME_CONTROLS.map(tc => (
                    <button key={tc.id} onClick={() => s.setTimeControl(tc.id)}
                      className={`p-3 rounded-lg text-left transition-colors ${s.timeControl === tc.id ? 'bg-red-600' : 'bg-slate-700 hover:bg-slate-600'}`}>
              ...
              <div className="flex items-center gap-3 mb-6">
                <button onClick={() => s.setRated(!s.rated)}
                  className={`w-10 h-6 rounded-full transition-colors ${s.rated ? 'bg-red-500' : 'bg-slate-600'}`}>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => navigator.clipboard?.writeText(s.inviteUrl!)}
                        className="px-3 py-2 bg-red-600 hover:bg-red-500 rounded text-sm">Copy</button>
                    </div>
      ...
      {(s.status === 'active' || s.status === 'ended') && checkersState && (
          ...
          <div className="flex flex-col gap-2 flex-1 min-w-0">
            ...
            {s.status === 'active' && (
              <div className="flex gap-2 justify-end">
                ...
                {checkersState.moveHistory.length < ABORT_MOVE_LIMIT ? (
          ...
          <div className="w-full lg:w-64 flex flex-col gap-4">
            ...
            <div className="bg-slate-800 rounded-xl p-4 flex flex-col gap-2 h-48">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={s.sendChat} className="px-3 py-1 bg-red-600 hover:bg-red-500 rounded text-sm">Send</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-slate-800 rounded-2xl p-8 text-center max-w-sm w-full mx-4 shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={s.playAgain} className="flex-1 py-3 bg-red-600 hover:bg-red-500 rounded-xl font-semibold">Play Again</button>

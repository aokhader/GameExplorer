...
export default function SpectateLobby() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white pt-16 flex flex-col items-center px-4 py-8">
      <div className="w-full max-w-2xl">
        ...
        <div className="bg-slate-800 rounded-2xl p-6 shadow-2xl mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold">Live now {games.length > 0 && <span className="text-slate-400">({games.length})</span>}</h2>
            <button onClick={loadGames} className="text-sm text-slate-400 hover:text-white">↻ Refresh</button>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-sm text-slate-400 py-8 text-center">No live games right now. Check back soon!</p>
          ) : (
            <div className="space-y-2">
              {games.map(g => (
                <button key={g.gameId} onClick={() => router.push(`/spectate/${g.gameId}`)}
                  className="w-full flex items-center justify-between bg-slate-700 hover:bg-slate-600 rounded-lg px-4 py-3 text-left transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-2xl">{GAME_ICON[g.gameType]}</span>
                    <div className="min-w-0">
                      <div className="font-medium truncate">
                        {g.white.username} ({g.white.rating}) <span className="text-slate-400">vs</span> {g.black.username} ({g.black.rating})
                      </div>
        ...
        <div className="bg-slate-800 rounded-2xl p-6 shadow-2xl">
          <p className="text-sm text-slate-400 mb-3">Have a game ID? Enter it to watch directly.</p>

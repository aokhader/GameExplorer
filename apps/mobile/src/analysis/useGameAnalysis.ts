...
export interface UseGameAnalysisOptions<S> {
  ...
  viewIndex: number;
  /** Side to move in a given state — the mover whose move gets graded. */
...
export function useGameAnalysis<S>({
  ...
  const grades = useMemo(() => {
    ...
    for (let i = 0; i < timeline.length - 1; i++) {
      ...
      const moverIsWhite = currentTurn(timeline[i]) === 'white';
    ...
    return out;
  }, [evals, timeline, adapter, currentTurn]);
  ...
  const summary = useMemo(() => {
    ...
    grades.forEach((g, i) => {
      if (!g) return;
      byColor[currentTurn(timeline[i])][g.grade]++;
    });
    return byColor;
  }, [grades, timeline, currentTurn]);

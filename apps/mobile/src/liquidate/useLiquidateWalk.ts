 ...
 *
 * Native port of web's hook of the same name, and it lives beside the game hook
 * for the same reason it does there: this is not only about drawing. A roll and
 * the move it causes land in the SAME engine update, so without a notion of "the
 * board has caught up", the property card appears while the piece is still
 * walking toward it, and a bot takes its next turn over the top of its own
 * animation. Both wait on `moving`.
 *
 * Only the placement is tracked here. The tween itself is reanimated's job in
 * `TokenLayer` — this hook just says which tile each piece is on *right now*.
 */
